...
API_URL="https://api.fraudguard.io/raw-lists-by-risk"
LOG_FILE="ufw_block_by_risk.log"
RISK_LEVEL=5 # Replace with your desired risk level
OFFSET=0
...
echo "Fetching IPs with risk level >= $RISK_LEVEL from FraudGuard.io..."
while true; do
    echo "Fetching from offset $OFFSET..."
    RESPONSE=$(curl -s -u "$USERNAME:$PASSWORD" -D - "${API_URL}/${RISK_LEVEL}?offset=${OFFSET}&limit=${LIMIT}&cidr=false&ipv6=false" -o /tmp/risk_ips.json)
    ...
    for IP in $(echo "$IP_LIST" | jq -r '.[]'); do
        ...
        if [[ $? -ne 0 ]]; then
            ufw deny from "$IP" comment "Blocked by FraudGuard.io (Risk Level $RISK_LEVEL)"
...
echo "UFW risk-based blocking complete!" | tee -a "$LOG_FILE"